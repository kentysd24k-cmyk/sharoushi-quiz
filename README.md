...
Cloudflare Pagesでホスティングしています。このGitHubリポジトリ(Private)と連携しており、`main`ブランチへの`git push`で自動的に再デプロイされます(ビルドコマンドなし、公開ディレクトリはリポジトリ直下)。検索エンジンにインデックスされないよう`index.html`に`noindex`を設定しています。
